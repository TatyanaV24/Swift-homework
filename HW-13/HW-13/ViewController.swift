...
extension Array {
    ...
    class ViewController: UIViewController {
        ...
        @IBAction func actionNext(_ sender: UIButton) {
            ...
            self.index = Swift.min(index+1, self.numberArray.count-1)
            self.numberArray[safeIndex: index]?.textColor = UIColor.blue
            
            if self.numberArray[index].textColor == UIColor.blue{
                self.numberArray[index-1].textColor = UIColor.black
            }
            changeView()
        }
        ...
        @IBAction func actionPrevious(_ sender: UIButton) {
            self.buttonsPressed.append(sender)
            self.index = Swift.max(0, index-1)
            self.numberArray[safeIndex: index]?.textColor = UIColor.red
            if self.numberArray[index].textColor == UIColor.red{
                self.numberArray[index+1].textColor = UIColor.black
            }
            changeView()
        }
